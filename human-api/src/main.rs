...
pub fn rocket() -> rocket::Rocket {
    ...
    .attach(AdHoc::on_attach("Solana client config", |rocket| {
        ...
        let human_protocol_program = String::from(rocket.config().get_str("human_protocol_program").unwrap());
        let offset = rocket.config().get_int("offset").unwrap() as usize;
        let config = Config {factory_version, rpc_client, human_protocol_program, offset};
...
mod test {
    ...
    pub struct MockedRpcClient { solana_client: Option<RpcClient>, mocks: Mocks }

    impl MockedRpcClient {
        pub fn new() -> Self {
            Self { solana_client: None, mocks: HashMap::new() }
        }

        pub fn create_rpc_client(&mut self) {
            self.solana_client = Some(RpcClient::new_mock_with_mocks(String::from(TEST_ENDPOINT), self.mocks.clone()));
        }
        ...
        pub fn mock_get_program_addresses_with_filter(&mut self) -> Pubkey {
            ...
            let ui_account = UiAccount::encode(&p_k, account, UiAccountEncoding::Base64, None, None);
            let response_data = vec![RpcKeyedAccount { pubkey: p_k.to_string(), account: ui_account }];
            self.mocks.insert(RpcRequest::GetProgramAccounts, json!(response_data));
            p_k
        }
    }
    ...
    fn test_rocket(mocked_client: MockedRpcClient) -> rocket::Rocket {
        ...
        .attach(AdHoc::on_attach("Solana client config", |rocket| {
            ...
            let config = Config {factory_version: FACTORY_VERSION, rpc_client: mocked_client.solana_client.unwrap(), human_protocol_program: String::from(HUMAN_PROTOCOL_PROGRAM), offset: OFFSET};
    ...
    fn test_get_factory_addresses() {
        ...
        let mut response = client.get(format!("/factory?address={}", HUMAN_PROTOCOL_PROGRAM)).dispatch();
